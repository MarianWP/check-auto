...
const TAG_UNIVERSE = new Set([...G.ENGINES.flatMap(e => e.tags), ...G.GEARBOXES.flatMap(g => g.tags)]);
...
describe("цілісність чек-листа", () => {
  ...
  it("теги в only і notes існують у довіднику моторів і коробок", () => {
    ALL_ITEMS.forEach(it => {
...
describe("фільтрація чек-листа під конфігурацію", () => {
  it("пункт з only видно лише коли всі його теги є в конфігурації; без only — завжди", () => {
    CONFIGS.forEach(cfg => {
      const tags = G.tagsFor(cfg);
      const visible = new Set(visibleStages(mkInsp(cfg)).flatMap(s => s.items).map(it => it.id));
      ALL_ITEMS.forEach(it => {
        const expected = !it.only || it.only.every(t => tags.has(t));
        expect(visible.has(it.id), JSON.stringify(cfg) + " " + it.id).toBe(expected);
      });
    });
  });

  it("немає «мертвих» пунктів: кожен умовний пункт видно хоча б в одній конфігурації", () => {
  ...
  it("у будь-якій конфігурації кожен етап непорожній і є критичні пункти", () => {
    CONFIGS.forEach(cfg => {
      const stages = visibleStages(mkInsp(cfg));
  ...
  it("фільтрація не змінює вихідний чек-лист", () => {
    ...
    visibleStages(mkInsp(CONFIGS[0]));
